...
| D018 | Implemented | 50-Scenario Automated Benchmark Runner (`BenchmarkRunner`) validates `tests/evaluations/cases.json` invariant suite (zero-coercion, temporal feasibility, price non-negativity) exposed via `GET /api/v1/benchmarks` and `/benchmarks` UI |
| D019 | Implemented | Interactive Geospatial Corridor Canvas using Leaflet with CartoDB Dark Matter styling matching SWENA `forest-night` (`#0D1915`/`#15271F`/`#B7C9AD`) with dynamic SSR-safe wrapper |
| D020 | Implemented | Client-native Web Speech API for voice brief dictation & audio tour narration; regional corridor weather & Western Ghats hazard advisory; public shareable dynamic route (`/trips/[id]`) with instant QR code generation |
| D021 | Implemented | SWYRA Auth (SGOD-pro/OAuth2.1) OAuth 2.1 / OIDC integration via Next.js BFF pattern with PKCE S256 (`/api/auth/*`), HttpOnly session cookies, and FastAPI RS256 Bearer offline JWKS validation |
| D022 | Implemented | Automated Playwright E2E test suite (`frontend/e2e/app.spec.ts`) validating marketing navigation, 5-tab constraint solving, corridor maps, benchmarks, mobile QR handover, and OAuth 2.1 session lifecycle integrated into `./verify.sh` |
